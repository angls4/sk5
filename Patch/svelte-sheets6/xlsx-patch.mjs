...
function parsexmltag(tag/*:string*/, skip_root/*:?boolean*/, skip_LC/*:?boolean*/)/*:any*/ {
  // console.log('parsexmltag , tag '+tag)
  var z = ({} /*:any*/);
  var eq = 0,
    c = 0;
  // loop until tag name ends
  ...
  if (m)
    for (i = 0; i != m.length; ++i) {
      ...
      // quot = 1 if character is quete or double-quote , else 0
      quot = (eq = cc.charCodeAt(c + 1)) == 34 || eq == 39 ? 1 : 0;
      // v = " "attrValue" " or " attrvalue "
      v = cc.slice(c + 1 + quot, cc.length - quot);
      // loop attrName (q) until colon ":" or eol
      for (j = 0; j != q.length; ++j) if (q.charCodeAt(j) === 58) break;
      // if eol
      if (j === q.length) {
        if (q.indexOf("_") > 0) q = q.slice(0, q.indexOf("_")); // from ods
        // assign z[attrName] = attrValue
        z[q] = v;
...
// patch
function width2px(width) { return Math.floor(width*COL2PX); }
// function width2px(width) { return Math.floor((COL2PXm * width + Math.round(128 / MDW) / 256) * MDW); }
	...
	(t[0].match(tagregex)||[]).forEach(function(x) {
		...
		switch(strip_ns(y[0])) {
			...
			case '<left': case '<left>': 
				...
				border.left = {};
				cb = border.left;
				if(y.style != undefined) cb.style = y.style;
				break;
			case '</left>': break;
			...
			case '<color': case '<color>':
				// patch
				if(y.rgb != undefined) cb.color = { rgb: y.rgb.slice(-6) };
				// black
				if(y.indexed == 64)cb.color = { rgb: "000000" };
				break;
...
function parse_fonts(t, styles, themes, opts) {
	...
	(t[0].match(tagregex)||[]).forEach(function(x) {
		...
		switch(strip_ns(y[0])) {
			...
			// patch
			case '<sz': if(y.val) font.sz = pt2px(+y.val); break;
...
// function safe_format(p/*:Cell*/, fmtid/*:number*/, fillid/*:?number*/, opts, themes, styles) {
	//patch
function safe_format(p/*:Cell*/, fmtid/*:number*/, fillid/*:?number*/, borderid, alignment, fontid, opts, themes, styles) {
	try {
    ...
    //patch
    // console.log(p.s.fgColor);
    // console.log(themes.themeElements.clrScheme);
    if (p.s.fgColor && p.s.fgColor.theme != undefined && !p.s.fgColor.rgb) {
      // console.log('tinted 1');
    ...
    // patch
    if (p.s.bgColor && p.s.bgColor.theme != undefined) {
      console.log("tinted 2");
    ...
    // patch
    // spread biar ga reference ke p.s = styles.Fills[fillid];
    if (Object.keys(styles.Borders[borderid]).length != 0) p.s = { ...p.s, borders: styles.Borders[Number(borderid)] };
    p.s = { ...p.s, alignment };
	const font = styles.Fonts[fontid];
	p.s = { ...p.s, font };
  } catch(e) { console.log(e);if(opts.WTF && styles.Fills) throw e; }
	// console.log(p);
}
...
// patch - regex untuk conditionalFormatting
var cfregex = /<(?:\w:)?conditionalFormatting[^>]*(?:[\/]|>([\s\S]*)<\/(?:\w:)?conditionalFormatting)>/;
// patch - parse cf
var parse_ws_xml_cf = (function() {
	var fregex = /<(?:\w+:)?formula[ \/>]/, ruleregex = /<\/(?:\w+:)?cfRule>/;
	var typeregex = /type=["']([^"']*)["']/, isregex = /<(?:\w+:)?is>([\S\s]*?)<\/(?:\w+:)?is>/;
	var idregex = /dxfId=["']([^"']*)["']/, pregex = /priority=["']([^"']*)["']/;
	var opregex = /operator=["']([^"']*)["']/;
	var match_v = matchtag("v"), match_f = matchtag("f");

return function parse_ws_xml_cf(sdata, s, opts, guess, themes, styles) {
...
function parse_ws_xml(data/*:?string*/, opts, idx/*:number*/, rels, wb/*:WBWBProps*/, themes, styles)/*:Worksheet*/ {
	// console.log('parse_ws_xml, data '+data)
	// parse bagian dari file-file worksheets/*.xml satu satu disini
	if(!data) return data;
	if(!rels) rels = {'!id':{}};
	...
	if(svs && svs[1]) parse_ws_xml_sheetviews(svs[1], wb, idx/*patch*/);
	...
	// patch
	var formatpr = data1.match(sheetformatprregex);
	  if (formatpr) s["!formatpr"] = parse_ws_xml_sheetformatpr(parsexmltag(formatpr[0]));
...
function parse_ws_xml_sheetpr(sheetPr/*:string*/, s, wb/*:WBWBProps*/, idx/*:number*/) {
	console.log('parse_ws_xml_sheetpr')
	console.log(sheetPr);
...
// patch
function parse_ws_xml_sheetformatpr(formatpr) {
	var o = {};
	if (formatpr["defaultRowHeight"]) o["defaultRowHeight"] = pt2px(parseFloat(formatpr["defaultRowHeight"]));
	if (formatpr["defaultColWidth"]) o["defaultColWidth"] = width2px(parseFloat(formatpr["defaultColWidth"]));
	return o;
}
...
// patch
function parse_ws_xml_sheetFormatPr(data, wb/*:WBWBProps*/,idx/*patch*/) {
	console.log("parse_ws_xml_sheetviews");
	...
	(data.match(sviewregex)||[]).forEach(function(r/*:string*/, i/*:number*/) {
		...
		// $FlowIgnore
		if(+tag.zoomScale) wb.Views[idx].zoom = +tag.zoomScale;
		// $FlowIgnore
		if(parsexmlbool(tag.rightToLeft)) wb.Views[idx].RTL = true;
		// patch
		if (tag.showGridLines){
			wb.Views[idx].showGridLines = parsexmlbool(tag.showGridLines);
		}
      // if (!parsexmlbool(tag.showGridLines))
	});
}
...
function parse_ws_xml_sheetviews(data, wb/*:WBWBProps*/,idx/*patch*/) {
	console.log("parse_ws_xml_sheetviews");
	if(!wb.Views) wb.Views = [{}];
	// patch
	if (!wb.Views[idx]) wb.Views[idx] = {};
...
return function parse_ws_xml_data(sdata/*:string*/, s, opts, guess/*:Range*/, themes, styles) {
	...
	var tag, tagr = 0, tagc = 0;
	var sstr, ftag;
	var fmtid = 0, fillid = 0, borderid = 0, fontid = 0;
	var alignment = {};
	var do_format = Array.isArray(styles.CellXf), cf;
	...
	for(var marr = sdata.split(rowregex), mt = 0, marrlen = marr.length; mt != marrlen; ++mt) {
		...
		if(rstarti >= ri) break;
		//parse c (cell) (row's child)
		tag = parsexmltag(x.slice(rstarti,ri), true);
		tagr = tag.r != null ? parseInt(tag.r, 10) : tagr+1; tagc = -1;
		// if read exceeds opts row limit
		if(opts.sheetRows && opts.sheetRows < tagr) continue;
		if(guess.s.r > tagr - 1) guess.s.r = tagr - 1;
		if(guess.e.r < tagr - 1) guess.e.r = tagr - 1;
		// console.log(guess)
		...
		for(var rslice = 0; rslice != cells.length; ++rslice) if(cells[rslice].trim().charAt(0) != "<") break;
		cells = cells.slice(rslice);
		// loop through row's cells
		for(ri = 0; ri != cells.length; ++ri) {
			x = cells[ri].trim();
			if(x.length === 0) continue;
			// console.log(x) = r="A1" s="1" t="s"><v>0</v></c> or r="C1" s="3"/>
			// cref = ['r="A1"',"A1"]
			cref = x.match(rregex); idx = ri; i=0; cc=0;
			x = "<c " + (x.slice(0,1)=="<"?">":"") + x;
			// console.log(x) = <c r="B1" s="2" t="s"><v>1</v></c> or <c r="C1" s="3"/>
			if(cref != null && cref.length === 2) {
				idx = 0; d=cref[1];
				for(i=0; i != d.length; ++i) {
					if((cc=d.charCodeAt(i)-64) < 1 || cc > 26) break;
			...
			// tagc = column letters to number
			// console.log(tagc)
			// loop until ">"
			for(i = 0; i != x.length; ++i) if(x.charCodeAt(i) === 62) break; ++i;
			tag = parsexmltag(x.slice(0,i), true);
			// console.log(tag) = {r: 'A1', s: '1', t: 's'}
			if(!tag.r) tag.r = encode_cell({r:tagr-1, c:tagc});
			d = x.slice(i);
			// console.log(d) = <v>0</v></c>
			p = ({t:""}/*:any*/);

			if((cref=d.match(match_v))!= null && /*::cref != null && */cref[1] !== '') p.v=unescapexml(cref[1]);
			// console.log(cref) = "vInner"
			if(opts.cellFormula) {
			...
			// fmtid = fillid = 0;
			// patch
			fmtid = fillid = borderid = fontid = 0;
			alignment = {};
			cf = null;
			if(do_format && tag.s !== undefined) {
				cf = styles.CellXf[tag.s];
			...
			// safe_format(p, fmtid, fillid, opts, themes, styles);
			// patch
			safe_format(p, fmtid, fillid, borderid, alignment, fontid, opts, themes, styles);
...
function parse_content_xml(d/*:string*/, _opts)/*:Workbook*/ {
	console.log("parse_content_xml , d "+d);
		var opts = _opts || {};
		if(DENSE != null && opts.dense == null) opts.dense = DENSE;
		var str = xlml_normalize(d);
...
function safe_parse_sheet(zip, path/*:string*/, relsPath/*:string*/, sheet, idx/*:number*/, sheetRels, sheets, stype/*:string*/, opts, wb, themes, styles) {
	console.log('safe_parse_sheet , path '+path);
	try {
