...
let xModifier = 0.02;
let yModifier = 0.02;
export function modifier(x, y) {
    xModifier = x;
    yModifier = y;
}
export let jexcelStyles = "max-width : min-content; max-height : min-content;";
export let classes = '';
export let classes2 = '';
export let data = [];
export let columns = [];
...
function refresh(data, viewport_height, viewport_width) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        //   x += 1;
        // }
        endX = columns.length + 10;
        // console.log(data.length)
        // let remains = columns.length;
...
$: {
    if (extension && extended) {
        ...
        topextend.style = `width: ${(right - left) * xModifier}px; left: ${left}px; top: ${top}px`;
        rightextend.style = `height: ${(bottom - top) * yModifier}px; left: ${right}px; top: ${top}px`;
        bottomextend.style = `width: ${(right - left) * xModifier}px; left: ${left}px; top: ${bottom}px`;
        leftextend.style = `height: ${(bottom - top) * yModifier}px; left: ${left}px; top: ${top}px`;
    }
}
let selectWidth;
let selectHeight;
      ...
      <tbody class="draggable" bind:this={viewport} on:scroll={handle_scroll}>
        {#each visibleY as r}
          <tr
            class="virtual-row"
            data-y={r.i}
            style={`height: ${getRowHeight(r.i)*yModifier}px`}
          >
            <th
              data-y={r.i}
              class:selected={selected &&
