...
function tabGroupScroll(e: WheelEvent) {
    e.preventDefault();
    this.scrollBy({ left: e.deltaY * 0.3, behavior: 'instant' });
}
</script>

<!-- TODO: Remove for V2 -->
<!-- svelte-ignore a11y-no-static-element-interactions -->
<div class="tab-group {classesBase}" data-testid="tab-group"  on:keypress on:keydown on:keyup>
	<!-- Tab List -->
	<div class="tab-list {classesList}" role="tablist" aria-labelledby={labelledby} on:mousewheel={tabGroupScroll} on:mouseenter on:mouseleave>
		<slot />
	</div>
	<!-- Tab Panel -->
	{#if $$slots.panel}
